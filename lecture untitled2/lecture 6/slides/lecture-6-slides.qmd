...
::: {.notes}
Source and code: Tutorial 6.1, `6.1-unsupervised-learning-features-distances.qmd`. Numerical examples use the bundled datasets or the explicitly identified simulation described there. Treat ratings as conditional on the stated model and sample.
:::
...
::: {.notes}
Source and code: Tutorial 6.2, `6.2-kmeans-and-kmedoids.qmd`. Numerical examples use the bundled datasets or the explicitly identified simulation described there. Treat ratings as conditional on the stated model and sample.
:::
...
::: {.notes}
Source and code: Tutorial 6.3, `6.3-hierarchical-clustering.qmd`. Numerical examples use the bundled datasets or the explicitly identified simulation described there. Treat ratings as conditional on the stated model and sample.
:::
...
::: {.notes}
Source and code: Tutorial 6.4, `6.4-gaussian-mixture-models.qmd`. Numerical examples use the bundled datasets or the explicitly identified simulation described there. Treat ratings as conditional on the stated model and sample.
:::
...
::: {.notes}
Source and code: Tutorial 6.5, `6.5-density-graphs-matrix-factorization.qmd`. Numerical examples use the bundled datasets or the explicitly identified simulation described there. Treat ratings as conditional on the stated model and sample.
:::
...
Clusters are connected sets of core points plus their borders. No $k$. With self-counting `minPts = m`, inspect the $(m-1)$-th other-neighbor distances.
...
::: {.notes}
Source and code: Tutorial 6.6, `6.6-validating-stabilizing-communicating.qmd`. Numerical examples use the bundled datasets or the explicitly identified simulation described there. Treat ratings as conditional on the stated model and sample.
:::
...
## 6.7 Guided Lab: Fuzzy Roles and Uncertain Profiles {.section-slide}

Shot-share geometry, graded memberships, and consensus under measurement uncertainty.

## 6.7.1 Shot selection as a composition

One player-season per row. Three attempt categories: rim, other two-point shots, and three-point shots.

$$
\sum_{z=1}^3p_{iz}=1,\qquad
\operatorname{clr}(\mathbf p_i)_z
=\log p_{iz}-\frac13\sum_{v=1}^3\log p_{iv}.
$$

Euclidean distance between CLR vectors compares relative allocation. CLR coordinates sum to zero.

::: {.notes}
Tutorial 6.7 uses NBA 2023–24 shot data stored locally. Rim means within 4 feet, with the three-point indicator taking precedence. At least 200 recorded shots defines eligibility. CLR geometry is based on Aitchison's compositional-data framework.
:::

## 6.7.2 Zeros and the coordinate dimension

```r
P <- (counts + 0.5) / (rowSums(counts) + 3 * 0.5)
Z <- log(P) - rowMeans(log(P))
```

- Smoothing corresponds to a Dirichlet prior with 0.5 per category.
- Three shares supply two independent coordinates.
- Full-covariance GMMs need a nonsingular contrast representation.

The lab preserves Aitchison distance by avoiding separate standardization of CLR columns.

## 6.7.3 Fuzzy c-means

$$
\min_{U,V}\sum_i\sum_k u_{ik}^{m}\|\mathbf z_i-\mathbf v_k\|^2,
\qquad\sum_ku_{ik}=1,\quad m>1.
$$
...
Fuzzy weights describe proximity under this objective. Multiple starts help assess local solutions.

## 6.7.4 Three different membership summaries

| Summary | Meaning |
|---|---|
| Fuzzy c-means weight | Weight in a distance-based objective |
| GMM responsibility | Conditional component probability under a fitted mixture |
| HDBSCAN membership strength | Density-based strength for an assigned cluster |

These quantities arise from different assumptions. None alone measures uncertainty from a limited number of shots.

## 6.7.5 Uncertainty in the input profile
...
Draw a shot-selection profile for every player, transform to CLR coordinates, and refit clustering. Repeat.

## 6.7.6 Consensus without label matching

$$
C_{ij}=\frac1B\sum_b\mathbb 1\{z_i^{(b)}=z_j^{(b)}\}.
$$

```r
C <- C + outer(labels, labels, "==") / B
consensus <- cutree(
  hclust(as.dist(1 - C), method = "average"), k = 4
)
```

The frequency describes this clustering procedure under posterior profile draws. It is not a fitted Bayesian mixture posterior.

## 6.7.7 Two perturbation questions

| Experiment | What changes? | Question |
|---|---|---|
| Dirichlet posterior draws | Shot-selection proportions | Are roles sensitive to limited attempts? |
| Player subsampling | Roster used to fit centers | Are boundaries sensitive to the comparison population? |

Resampling complete games can address within-player game dependence. A dynamic model can address role changes over time.

## 6.7.8 Lab deliverable

A one-page scouting summary with eligibility, feature definitions, centers in original units, two ambiguous players, and a stability statement.

The four-cluster lab solution is a teaching resolution. Compare other values before using it in a scouting workflow.

::: {.notes}
The tutorial supplies code and discussion answers. Students should name a cluster after examining its representative share profile and should distinguish shot selection from shooting skill. References: e1071 cmeans manual, dbscan HDBSCAN documentation, and Aitchison (1986).
:::


## 6.7.9 Readings and tutorial code

The matching tutorials contain derivations, executable R code, data provenance, and discussion answers.

- [mclust documentation](https://mclust-org.github.io/mclust/)
- [HDBSCAN in R](https://search.r-project.org/CRAN/refmans/dbscan/html/hdbscan.html)
- [Fuzzy c-means in e1071](https://cran.r-project.org/web/packages/e1071/e1071.pdf)
