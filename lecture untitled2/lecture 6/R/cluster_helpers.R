...
# Resampling perturbation inspired by cluster stability. Duplicate sampled
# indices are removed, so this is not the ordinary multiplicity-preserving
# bootstrap. Scaling stays fixed. For each distinct-row sample,
...
em_gmm <- function(X, k, max_iterations = 200, tolerance = 1e-6, seed = 431) {
  ...
  # Recompute the E-step after the last M-step so returned assignments,
  # likelihood, and BIC refer to the returned parameter estimates.
  densities <- sapply(seq_len(k), function(j)
    weights[[j]] * mvnormal_density(X, means[j, ], covariances[[j]]))
  total <- rowSums(densities)
  if (any(!is.finite(total) | total <= 0))
    stop("Nonfinite mixture density. Use log-scale densities or a package fit.")
  responsibilities <- densities / total
  log_likelihood <- c(log_likelihood, sum(log(total)))
...
spectral_clustering <- function(X, k, neighbors = 10, seed = 431) {
  n <- nrow(X)
  D <- as.matrix(dist(X))
  # Symmetrized union k-nearest-neighbor affinity with a local Gaussian kernel
