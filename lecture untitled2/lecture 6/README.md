...
The included `data/nba_2024.rds` file keeps rendering fast and offline. Run `R/prepare_nba_data.R` only when you want to rebuild the data from ESPN through hoopR. No model fits are precomputed; runtime varies by machine and tutorial.
...
`6.7-guided-lab-fuzzy-compositional-consensus.qmd` adds guided lab: fuzzy roles and uncertain profiles. The extracted script `R/guided_lab.R` runs from this lecture folder. Install `posterior` for Lectures 4 and 5, and `e1071` for Lecture 6. The new Gibbs examples run in R without Stan.
