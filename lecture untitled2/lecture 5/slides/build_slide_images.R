...
bayes_fit <- stan_glm(
  ...
  prior = normal(0, c(3, rep(fit_margin_hyperparameters(games_2023)$tau, ncol(X))), autoscale = FALSE), prior_aux = exponential(1 / 13),
...
# Evaluation uses prespecified parameters, not full-period fitted parameters.
kalman <- kalman_ratings(nfl, sigma_obs = 13, sigma_week = 1.2,
  sigma_season = 4, rho = 0.6, home_advantage = 2, initial_sd = 6)
