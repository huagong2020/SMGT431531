...
::: {.notes}
Source and code: Tutorial 5.1, `5.1-team-evaluation-outcomes-luck-schedules.qmd`. Numerical examples use the bundled datasets or the explicitly identified simulation described there. Treat ratings as conditional on the stated model and sample.
:::
...
::: {.notes}
Source and code: Tutorial 5.2, `5.2-ratings-from-margins.qmd`. Numerical examples use the bundled datasets or the explicitly identified simulation described there. Treat ratings as conditional on the stated model and sample.
:::
...
## 5.2.6 Bayesian regression at a fixed prior scale
...
::: {.notes}
Source and code: Tutorial 5.3, `5.3-bradley-terry-win-loss-ratings.qmd`. Numerical examples use the bundled datasets or the explicitly identified simulation described there. Treat ratings as conditional on the stated model and sample.
:::
...
::: {.notes}
Source and code: Tutorial 5.4, `5.4-bayesian-poisson-goal-model.qmd`. Numerical examples use the bundled datasets or the explicitly identified simulation described there. Treat ratings as conditional on the stated model and sample.
:::
...
::: {.notes}
Source and code: Tutorial 5.5, `5.5-dynamic-ratings-elo-state-space.qmd`. Numerical examples use the bundled datasets or the explicitly identified simulation described there. Treat ratings as conditional on the stated model and sample.
:::
...
::: {.notes}
Source and code: Tutorial 5.6, `5.6-evaluating-communicating-ratings.qmd`. Numerical examples use the bundled datasets or the explicitly identified simulation described there. Treat ratings as conditional on the stated model and sample.
:::
...
|---|---:|---:|---:|
| 2022 home-win baseline | 0.690 | 0.248 | 0.544 |
| Rolling ridge | 0.649 | 0.229 | 0.614 |
| Elo, margin multiplier | 0.639 | 0.224 | 0.632 |
| Kalman, fixed parameters | 0.639 | 0.224 | 0.634 |
| Market: closing spread | 0.609 | 0.210 | 0.697 |
| Market: normalized moneyline | 0.607 | 0.209 | 0.699 |
...
## 5.7 Guided Lab: Bayesian Team Forecasting {.section-slide}

A full hierarchy for team strength and predictions for games after Week 10.

## 5.7.1 The model in points

$$
\begin{aligned}
\mathbf y\mid\alpha,\boldsymbol\theta,\sigma^2
&\sim N(\alpha\mathbf h+X\boldsymbol\theta,\sigma^2I),\\
\theta_j\mid\tau^2&\sim N(0,\tau^2),\quad\alpha\sim N(0,9),\\
\tau^2&\sim\operatorname{IG}(2,25),\quad
\sigma^2\sim\operatorname{IG}(2,169).
\end{aligned}
$$

One game per row. $X$: +1 for the home team, âˆ’1 for the away team. $h=0$ at neutral sites.

::: {.notes}
Tutorial 5.7 defines inverse-gamma using a reciprocal gamma with rate parameter. All normal second arguments are variances. The priors are teaching assumptions. Data are the locally stored nflverse games.
:::

## 5.7.2 Ridge and full Bayes

Conditional on the population parameters,

$$
\mathbf m_\theta=(X^{\mathsf T}X+\lambda I)^{-1}
X^{\mathsf T}(\mathbf y-\alpha\mathbf h),
\qquad\lambda=\sigma^2/\tau^2.
$$

- Empirical Bayes fixes estimated $\sigma$ and $\tau$.
- Full Bayes samples them and carries their uncertainty into the ratings.

The proper prior identifies a common level. The lab centers draws for display only.

## 5.7.3 Joint draws preserve opponent uncertainty
...
```r
delta <- draws[, "KC"] - draws[, "BUF"]
quantile(delta, c(.025, .975))
mean(delta > 0)
```

Subtracting marginal interval endpoints does not produce the comparison interval.

## 5.7.4 Strength and a future win

For posterior draw $s$,

$$
\eta^{(s)}=\alpha^{(s)}h_*+\theta_H^{(s)}-\theta_A^{(s)},\qquad
P(\widetilde y>0\mid y)\approx\frac1S\sum_s\Phi(\eta^{(s)}/\sigma^{(s)}).
$$

The probability of winning includes venue and game noise. The probability of greater strength concerns the parameters.

## 5.7.5 The forecast timeline

| Stage | Available information | Output |
|---|---|---|
| Fit at Week 10 | 2023 Weeks 1â€“10 | Joint posterior |
| Forecast | Upcoming matchups and venue | Win probabilities and margin intervals |
| Evaluate | Later completed games | Brier score, log loss, RMSE, coverage |

The lab keeps one Week-10 fit. Weekly refitting answers a different operational question.

## 5.7.6 Predictive checking and test scoring

**Posterior predictive check:** replicate training games and compare extreme margins with those observed.

**Test scoring:** predict later games and compare with outcomes withheld from fitting.

The continuous margin model gives zero probability to an exact tie. The binary score demonstration uses decisive games and states that target.

## 5.7.7 Worked conditional forecast

Suppose home advantage is 2 points and the strength difference is 4 points, with game standard deviation 13 points.

$$
P(\text{home win}\mid\alpha,\boldsymbol\theta,\sigma)
=\Phi(6/13)\approx0.678.
$$

Full Bayes averages this calculation over joint parameter draws. It also returns an interval for the realized margin.

## 5.7.8 Lab deliverable

A team comparison and a future-game forecast, each with its own uncertainty statement.

Explain the information cutoff, the prior, and whether the rating is static, filtered, or smoothed.

::: {.notes}
Students should distinguish a latent comparison interval from a predictive margin interval. Tutorial 5.7 contains the executable workflow and discussion answers. A forecast using full-season hyperparameters would violate the Week-10 information cutoff.
:::


## 5.7.9 Readings and tutorial code
