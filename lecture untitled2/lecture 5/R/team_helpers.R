...
# Elo ratings
# ---------------------------------------------------------------------------
fit_margin_hyperparameters <- function(games) {
  X <- build_design_matrix(games)
  h <- 1 - games$neutral_site
  J <- ncol(X)
  N <- nrow(X)
  XtX <- crossprod(X)
  objective <- function(par) {
    tau2 <- exp(2 * par[2])
    sigma2 <- exp(2 * par[3])
    r <- games$margin - par[1] * h
    b <- crossprod(X, r)
    A <- XtX + sigma2 / tau2 * diag(J)
    logdet <- (N - J) * log(sigma2) +
      as.numeric(determinant(sigma2 * diag(J) + tau2 * XtX)$modulus)
    quad <- (sum(r^2) - as.numeric(crossprod(b, solve(A, b)))) / sigma2
    0.5 * (logdet + quad + N * log(2 * pi))
  }
  fit <- optim(c(2, log(5), log(13)), objective, method = "L-BFGS-B",
               lower = c(-15, log(0.01), log(0.1)),
               upper = c(15, log(30), log(50)))
  stopifnot(fit$convergence == 0)
  list(home_advantage = fit$par[1], tau = exp(fit$par[2]),
       sigma = exp(fit$par[3]), lambda = exp(2 * (fit$par[3] - fit$par[2])))
}
