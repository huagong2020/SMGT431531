...
Every estimate in this lecture has the form

$$
\hat\theta_j=\bar y+w_j\left(\bar y_j-\bar y\right),
\qquad
...
## 4.4.9 What full Bayes adds

Integrate $\theta_j$ out: $\bar y_j\sim N(\mu,\ \tau^2+\sigma^2/n_j)$ gives a marginal likelihood for $\tau$. Put a grid on $\tau$, and average each passer's normal posterior over it.

- With about 100 passers the posterior of $\tau$ is well concentrated and the two answers are close.
- With 10 teams or 3 seasons the difference is large, and the fully Bayesian interval is the honest one.
...
| REML or ML for $\tau^2$ | marginal likelihood mode (empirical Bayes) |
...
## 4.5.17 Empirical Bayes and full Bayes agree on the points

![](images/stan-vs-lme4.png){fig-align="center" width="66%"}

::: {.small}
With plentiful data the posterior means match `lme4`. The value of the draws is in what they let us compute next.
:::
