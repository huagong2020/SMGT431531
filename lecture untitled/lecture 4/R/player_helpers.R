...
logistic_log_posterior <- function(beta, X, y, prior_sd = 2.5) {
  ...
  log_likelihood <- sum(y * eta - log1p(exp(eta)))
...
# Effective sample size for one chain by the initial positive sequence method
# (a simplified version of the estimator used by Stan).
